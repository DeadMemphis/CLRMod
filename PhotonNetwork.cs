...
public static class PhotonNetwork
{
    ...
    public static PeerStates connectionStatesDetailed
    {
        get
        {
            ...
            if (offlineMode)
            {
                ...
                if (flag)
                {
                    result = PeerStates.Joined;
                }
                else
                {
                    result = PeerStates.ConnectedToMaster;
                }
            }
            else
            {
                result = PhotonNetwork.networkingPeer.states;
            }
            return result;
        }
    }
    ...
    {
        switch (PhotonNetwork.networkingPeer.UsedProtocol)
        {
    ...
    internal static void EventRPC(PhotonView view, string methodName, RaiseEventOptions options, params object[] parameters)
    {
        if (PhotonNetwork.VerifyCanUseNetwork())
        {
            ...
            {
                PhotonNetwork.networkingPeer.EventRPC(view, methodName, options, parameters);
                return;
            }
    ...
    public static T Instantiate<T>(string prefabName, Vector3 position, Quaternion rotation, int group = 0, object[] data = null) where T : Component
    {
        ...
        int[] array = new int[gameObject.GetComponentsInChildren<PhotonView>(true).Length];
    ...
    public static void PoolDisable(this CLEARSKIES.Pool pool, GameObject GO)
    {
        if (pool != null && GO != null && pool.Contains(GO))
        {
            ...
            PhotonNetwork.networkingPeer.PoolDisable(GO, false);
            PhotonNetwork.UnAllocateViewID(component.viewID);
            GO.SetActive(false);
        }
    }
    public static T PoolEnable<T>(this CLEARSKIES.Pool pool, string prefabName, Vector3 position, Quaternion rotation, int group = 0, object[] data = null) where T : Component
    {
        ...
        if (pool == null)
        {
            Debug.LogError("Failed to enable an object when the pool is null.");
            return default(T);
        }
        T t = pool.Enable<T>(prefabName, position, rotation, true);
        if (t == null)
        {
            return default(T);
        }
        PhotonView component = t.GetComponent<PhotonView>();
        if (component == null)
        {
            pool.Disable(t, true);
        ...
        PhotonNetwork.networkingPeer.instantiatedObjects[component.viewID] = t.gameObject;
        PhotonNetwork.networkingPeer.photonViewList.Remove(component.viewID);
        PhotonNetwork.UnAllocateViewID(component.viewID);
        PhotonNetwork.networkingPeer.SendInstantiate(prefabName, position, rotation, group, PhotonView.CreateNewViewID(prefabName, component), data, false);
    ...
    public static GameObject PoolEnable(this CLEARSKIES.Pool pool, string prefabName, Vector3 position, Quaternion rotation, int group = 0, object[] data = null)
    {
        ...
        if (pool == null)
        {
            Debug.LogError("Failed to enable prefab: " + prefabName + " when the pool is null.");
            return null;
        }
        GameObject gameObject = pool.Enable(prefabName, position, rotation, true);
        if (gameObject == null)
        {
            return null;
        }
        ...
        if (component == null)
        {
            pool.Disable(gameObject, true);
            Debug.LogError("Failed to Instantiate prefab:" + prefabName + ". Prefab must have a PhotonView component.");
            return null;
        }
        PhotonNetwork.networkingPeer.instantiatedObjects[component.viewID] = gameObject;
        PhotonNetwork.networkingPeer.photonViewList.Remove(component.viewID);
        PhotonNetwork.UnAllocateViewID(component.viewID);
        PhotonNetwork.networkingPeer.SendInstantiate(prefabName, position, rotation, group, PhotonView.CreateNewViewID(prefabName, component), data, false);
        PhotonNetwork.networkingPeer.photonViewList[component.viewID] = component;
    ...
    public static T PoolEventEnable<T>(this CLEARSKIES.Pool pool, string prefabName, Vector3 position, Quaternion rotation, RaiseEventOptions options) where T : Component
    {
        ...
        PhotonNetwork.networkingPeer.photonViewList.Remove(component.viewID);
        PhotonNetwork.UnAllocateViewID(component.viewID);
        PhotonNetwork.networkingPeer.SendEventInstantiate(prefabName, position, rotation, PhotonView.CreateNewViewID(prefabName, component), options, 0, null);
        PhotonNetwork.networkingPeer.photonViewList[component.viewID] = component;
        return t;
    }
