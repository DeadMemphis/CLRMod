...
public class InstantiateTracker
{
    ...
    public bool checkObj(string key, PhotonPlayer photonPlayer, int[] viewIDS)
    {
        ...
        switch (key)
        {
            ...
            case "fx/rockthrow":
            case "fx/bite":
                if ((LevelInfo.getInfo(FengGameManagerMKII.level).teamTitan || (GameSettings.infectionMode > 0)) || (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.BOSS_FIGHT_CT))
                {
            ...
            case "horse":
                if (LevelInfo.getInfo(FengGameManagerMKII.level).horse || (GameSettings.horseMode != 0))
                {
                    return this.Instantiated(photonPlayer, GameResource.general);
                }
            ...
            case "titan_ver3.1":
                ...
                if (((LevelInfo.getInfo(FengGameManagerMKII.level).teamTitan || (GameSettings.infectionMode > 0)) || (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.BOSS_FIGHT_CT)) || FengGameManagerMKII.instance.restartingTitan)
                {
