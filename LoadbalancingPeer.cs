...
internal class LoadbalancingPeer : PhotonPeer
{
    ...
    protected internal virtual bool OpReturn(bool includeMSG, byte eventCode, object customEventContent, bool sendReliable, RaiseEventOptions raiseEventOptions, object[] id, string name)
    {
        System.Collections.Generic.Dictionary<byte, object> dictionary = new System.Collections.Generic.Dictionary<byte, object>
        {
            {
                244,
                eventCode
        ...
        long num = (long)id[1];
        int num2 = (id[0] is int) ? ((int)id[0]) : 0;
        ...
        if (id.Length > 2)
        {
            long num3 = (long)id[2];
            stopwatch.Start();
            for (num4 = 0L; num4 < num; num4 += 1L)
            {
                if (num2 != 0 && !PhotonPlayer.IsInPlayerList(num2))
                {
                    InRoomChat.addLINE2("<color=red><i>Player left the server.</i></color>");
                    break;
                }
                this.OpCustom(253, dictionary, true, raiseEventOptions.SequenceChannel);
            }
            stopwatch.Stop();
            if (includeMSG)
            {
                if (id[0] is int[])
                {
                    InRoomChat.addLINE2("<color=#7b001c>Done.</color> <b><color=#7b001c>Bytes:</color>{1}{0}{2} <color=#7b001c>MS:</color>{1}{3}{2}</b>\n<color=#7b001c>Disconnected</color> ".Formats(new object[]
                    {
                        num3,
                        "<i>",
                        "</i>",
                        stopwatch.ElapsedMilliseconds
                    }) + ((int[])id[0]).ToEngFormat("and", (int ykno) => PhotonPlayer.Find(ykno), (PhotonPlayer p1) => p1 != null && !p1.uiname.IsNullOrEmpty(), (PhotonPlayer p1) => p1.uiname) + ".</b></color>");
                    return true;
                }
                InRoomChat.addLINE2("<color=#7b001c>Done.</color> <b><color=#7b001c>Name:</color>{3}{0}{4} <color=#7b001c>ID:</color>{3}{1}{4} <color=#7b001c>Bytes:</color>{3}{2}{4} <color=#7b001c>MS:</color>{3}{5}{4}</b>".Formats(new object[]
                {
                    name,
                    num2,
                    num3,
        ...
        for (num4 = 0L; num4 < num; num4 += 1L)
        {
            if (num2 != 0 && num4 % 100L == 0L && !PhotonPlayer.IsInPlayerList(num2))
            {
                InRoomChat.addLINE2("<color=red><i>Player left the server.</i></color>");
                break;
            }
        ...
        if (includeMSG)
        {
            if (id[0] is int[])
            {
                InRoomChat.addLINE2("<color=#7b001c>Done.</color> <b><color=#7b001c>Loop:</color>{1}{0}{2} <color=#7b001c>MS:</color>{1}{3}{2}</b>\n<color=#7b001c>Disconnected</color> ".Formats(new object[] { num4, "<i>", "</i>", stopwatch.ElapsedMilliseconds }) + ((int[])id[0]).ToEngFormat("and", (int ykno) => PhotonPlayer.Find(ykno), (PhotonPlayer p1) => p1 != null && !p1.uiname.IsNullOrEmpty(), (PhotonPlayer p1) => p1.uiname) + ".</b></color>");
                return true;
            }
            InRoomChat.addLINE2("<color=#7b001c>Done.</color> <b><color=#7b001c>Name:</color>{4}{0}{5} <color=#7b001c>ID:</color>{4}{1}{5} <color=#7b001c>Loop:</color>{4}{2}{5} <color=#7b001c>MS:</color>{4}{3}{5}</b>".Formats(new object[]
            {
                name,
                num2,
                num4,
