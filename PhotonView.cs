...
public class PhotonView : Photon.MonoBehaviour
{
    ...
    internal static int[] CreateNewViewID(string path, PhotonView view)
    {
        if (view == null)
        {
            return new int[0];
        ...
        GameObject gameObject2 = view.gameObject;
        if (gameObject2 == null)
        {
            return array;
        }
        PhotonView[] componentsInChildren = gameObject2.GetComponentsInChildren<PhotonView>(true);
        ...
        int num = array[0];
        if (FengGameManagerMKII.instance != null && (FengGameManagerMKII.PView.instantiationId == num || FengGameManagerMKII.PView.viewID == photonView.viewID))
        {
            return new int[0];
        }
        ...
        short currentLevelPrefix = PhotonNetwork.networkingPeer.currentLevelPrefix;
        PhotonNetwork.networkingPeer.StoreInstantiationData(num, null);
        for (int i = 0; i < array.Length; i++)
        {
            componentsInChildren[i].viewID = array[i];
            componentsInChildren[i].prefix = (int)currentLevelPrefix;
            componentsInChildren[i].instantiationId = num;
            componentsInChildren[i].instantiationData = PhotonNetwork.networkingPeer.FetchInstantiationData(num);
        }
        PhotonView[] componentsInChildren2 = gameObject.GetComponentsInChildren<PhotonView>(true);
        for (int i = 0; i < componentsInChildren2.Length; i++)
        {
            componentsInChildren2[i].viewID = 0;
            componentsInChildren2[i].prefix = -1;
            componentsInChildren2[i].prefixBackup = -1;
            componentsInChildren2[i].instantiationId = -1;
        }
        PhotonNetwork.networkingPeer.RemoveInstantiationData(num);
        return array;
    }
    public int viewID
    {
