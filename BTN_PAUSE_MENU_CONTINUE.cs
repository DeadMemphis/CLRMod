...
public class BTN_PAUSE_MENU_CONTINUE : MonoBehaviour
{
    private void OnClick()
    {
        ...
        if (!IN_GAME_MAIN_CAMERA.mainCamera.enabled)
        {
            ...
            BRM.CacheGameObject.Find("MainCamera").GetComponent<SpectatorMovement>().disable = false;
            BRM.CacheGameObject.Find("MainCamera").GetComponent<MouseLook>().disable = false;
        }
        else
        {
