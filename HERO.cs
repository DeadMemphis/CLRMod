...
public class HERO : MONO
{
    ...
    public bool isGassing
    {
        get
        {
            return !this.IsGrounded() && this.smoke_3dmg != null && this.smoke_3dmg.enableEmission;
        }
    }
    public bool isHooked
    {
        get
        {
            return (this.hook.ContainsKey("left") && this.hook["left"] != null && this.hook["left"].isHooked()) || (this.hook.ContainsKey("right") && this.hook["right"] != null && this.hook["right"].isHooked());
        }
    }
    ...
    public void blowAway(Vector3 force, PhotonMessageInfo info = null)
    {
        ...
        {
            StatsTab.AddLine(" [<color=black>Blocked blowAway</color>] [<color=#7DFF0D>[" + info.sender.ID + "]</color>" + info.sender.customProperties["name"].ToString().ToRGBA() + "] [<color=#261DF5>" + force.x + "," + force.y + "," + force.z + "</color>]", StatsTab.DebugType.WARNING);
        }
    }
    ...
    public void hookToHuman(GameObject target, Vector3 hookPosition, bool dash = false)
    {
        ...
        if (hero != null)
        {
            ...
            if(dash && !hero.photonView.isMine) PhotonNetwork.Instantiate("FX/boom6", hero.transform.position, hero.transform.rotation, 0);
        }
    ...
    private void launchDashRopes()
    {
        float posx = 5f, posz = 5f;
        Vector3 vect;

        //this.bulletRT.position = (!this.useGun ? this.hookRefR1T : this.hookRefR2T).position; 
        //Vector3 vector = (((baseT.transform.position * 0.5f) * (angle+=60f) + ((Vector3)((baseT.right * 0.05f) * angle))) - this.baseT.position);
        //vector.Normalize();
        //RaycastHit hit1;
        //vect = new Vector3(baseT.position.x + posx, baseT.position.y, baseT.position.z);
        //Ray ray1 = Camera.main.ScreenPointToRay(vect);
        //LayerMask mask0 = ((int)1) << LayerMask.NameToLayer("Ground");
        //LayerMask mask1 = ((int)1) << LayerMask.NameToLayer("EnemyBox");
        //LayerMask mask2 = mask1 | mask0;
        //if (Physics.Raycast(ray1, out hit1, 10000f, mask2.value))
        //{
        //    launchLeftRope(hit1, true, 0);          
        //}

        for (int i = 0; i < FengGameManagerMKII.allheroes.Count; i++)
        {
            //if (FengGameManagerMKII.allheroes[i].GetComponent<HERO>().photonView != basePV.isLocal)
            //{
                vect = (FengGameManagerMKII.allheroes[i].transform.position - (baseT.right * 0.3f)) - this.baseT.position;
                DashRampage[i] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
                DashRampage[i].launchDashNets(vect, baseR.velocity, baseG, FengGameManagerMKII.allheroes[i].GetComponent<HERO>());
            //}
            //vect = (new Vector3(baseT.position.x + posx, baseT.position.y, baseT.position.z) - baseT.right) - this.baseT.position;
            //DashRampage[0] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[0].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x - posx, baseT.position.y, baseT.position.z) - baseT.right) - this.baseT.position;
            //DashRampage[1] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[1].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x, baseT.position.y, baseT.position.z - posz) - baseT.right) - this.baseT.position;
            //DashRampage[2] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[2].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x, baseT.position.y, baseT.position.z + posz) - baseT.right) - this.baseT.position;
            //DashRampage[3] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[3].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x + posx, baseT.position.y, baseT.position.z - posz) - baseT.right) - this.baseT.position;
            //DashRampage[4] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[4].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x - posx, baseT.position.y, baseT.position.z + posz) - baseT.right) - this.baseT.position;
            //DashRampage[5] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[5].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x + posx, baseT.position.y, baseT.position.z + posz) - baseT.right) - this.baseT.position;
            //DashRampage[6] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[6].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
            //vect = (new Vector3(baseT.position.x - posx, baseT.position.y, baseT.position.z - posz) - baseT.right) - this.baseT.position;
            //DashRampage[7] = (PhotonNetwork.Instantiate("hook", baseT.position, baseT.rotation, 0)).GetComponent<Bullet>();
            //DashRampage[7].launchDashNets(vect, baseR.velocity * 1000f, baseG, FengGameManagerMKII.allheroes[0].GetComponent<HERO>());
    ...
    private void netGrabbed(int id, bool leftHand, PhotonMessageInfo info)
    {
        ...
        {
            StatsTab.AddLine("Blocked <color=yellow>netGrabbed</color> from " + info.sender.uiname.ToRGBA(), StatsTab.DebugType.WARNING);
        }
    }
    ...
    //                            this.suicide();
    //                        }
    //                        if (((this.myHorse != null) && this.isMounted) && FengCustomInputs.Inputs.isInputDown[InputCode.dodge])
    //                        {
    //                            this.getOffHorse();
    //                        }
    //                        if ((baseA.IsPlaying(this.standAnimation) || !this.grounded) && FengCustomInputs.Inputs.isInputDown[InputCode.reload])
    //                        {
    //                            this.changeBlade();
    //                            return;
    //                        }
    ...
    //                                }
    //                            }
    //                            if (flag5 || FengCustomInputs.Inputs.isInputUp[InputCode.attack0])
    //                            {
    //                                if (this.grounded)
    //                                {
    //                                    if (this.leftGunHasBullet && this.rightGunHasBullet)
    ...
    //                                    obj6.renderer.material = CharacterMaterials.materials[this.setup.myCostume._3dmg_texture];
    //                                    Vector3 vector4 = ((Vector3) ((-baseT.forward * 10f) + (baseT.up * 5f))) + baseT.right;
    ...
    //                        }
    //                    }
    //                    if (((FengCustomInputs.Inputs.isInput[InputCode.leftRope] && !baseA.IsPlaying("attack3_1")) && (!baseA.IsPlaying("attack5") && !baseA.IsPlaying("special_petra"))) && ((this.state != HERO_STATE.ChangeBlade) && (this.state != HERO_STATE.Grab)))
    //                    {
    //                        if (this.bulletLeft != null)
    //                        {
    //                            this.QHold = true;
    ...
    //                        this.QHold = false;
    //                    }
    //                    if (FengCustomInputs.Inputs.isInput[InputCode.rightRope] && (((!baseA.IsPlaying("attack3_1") && !baseA.IsPlaying("attack5")) && !baseA.IsPlaying("special_petra")) || (this.state == HERO_STATE.Idle)))
    //                    {
    //                        if (this.bulletRight != null)
    //                        {
    //                            this.EHold = true;
    ...
    //                        this.EHold = false;
    //                    }
    //                    if (FengCustomInputs.Inputs.isInput[InputCode.bothRope] && (((!baseA.IsPlaying("attack3_1") && !baseA.IsPlaying("attack5")) && !baseA.IsPlaying("special_petra")) || (this.state == HERO_STATE.Idle)))
    //                    {
    //                        this.QHold = true;
    //                        this.EHold = true;
    //                        if ((this.bulletLeft == null) && (this.bulletRight == null))
    ...
    {
        if (myhero == null && targetHero == null)
        {
            return;
        }
        ...
        if (myhero.isHooked)
        {
            ...
            if (myhero.isGassing)
            {
                Animation animation = targetHero.animation;
        ...
        else
        {
            ...
            if (myhero.isGassing)
            {
                Animation animation2 = targetHero.animation;
    ...
    public void RockThrow(out Vector3 launch, GameObject target)
    {
        ...
        if (target != null && this.rockT != null && this.throwRock != null)
        {
            ...
            float num3 = myHeroTlocal.position.y + myHeroRlocal.velocity.y + 2f * 3f;
            ...
            if (targetHERO.isGassing)
            {
                ...
                if (!animation.IsPlaying("attack5") && !animation.IsPlaying("special_petra") && !animation.IsPlaying("dash") && !animation.IsPlaying("jump") && !animation.IsPlaying("air_release") && !animation.IsPlaying("dodge"))
                {
                    ...
                    if (num6 >= 15f)
                    {
                        float num7 = targetHERO.isLocal ? 1.5f : 1.55f;
                        vector += myHeroTlocal.forward * ((float)targetHERO.setup.myCostume.stat.ACL / 10f * Mathf.Clamp(1f + num6 % 1.218f, 1f, num7));
                    }
                    else if (targetHERO.isLocal)
                    {
                        vector += vector.normalized * ((float)targetHERO.setup.myCostume.stat.ACL / 10f * 1f);
                    }
                    else
                    {
                        vector += vector.normalized * ((float)targetHERO.setup.myCostume.stat.ACL / 10f * 1.1f);
                    }
                }
            ...
            if ((animation.IsPlaying("horse_idle") || animation.IsPlaying("horse_run")) && target != null && targetHERO.horse != null)
            {
                ...
                num3 = transform.position.y + rigidbody.velocity.y + 2f * 3f;
            ...
            else if (target != null)
            {
                ...
                this.RockThrowAim1(angularDistance, targetHERO, num2, num, num3, num4, num5, ref launch);
                return;
            }
            else if ((bool)FengGameManagerMKII.settings[10])
            {
                ...
                {
                    this.RockThrowAim4(angularDistance, targetHERO, num2, num, num4, out launch);
                    return;
                }
                if (UnityEngine.Random.Range(0, 10) < 4)
                {
                    this.RockThrowAim2(angularDistance, targetHERO, num2, num, num4, out launch);
                    return;
                }
                this.RockThrowAim3(angularDistance, targetHERO, num2, num, num3, num4, num5, ref launch);
                return;
            }
                        ...
                        switch (state)
                        {
                            case HERO_STATE.Idle:
                                ...
                                if ((!this.isMounted && (FengCustomInputs.Inputs.isInputDown[InputCode.attack0] || FengCustomInputs.Inputs.isInputDown[InputCode.attack1])) && !this.useGun)
                                {
                                    ...
                                    //targetHero = getNearestHeroFocusControl();
                                    //if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && basePV.isMine)
                                    //{
                                    //    this.rockT = (this.throwRock = PhotonNetwork.Instantiate("FX/rockThrow", this.baseT.position,
                                    //            Quaternion.LookRotation(this.targetHero.transform.position + this.targetHero.rigidbody.velocity * 3f), 0).GetComponent<RockThrow>()).transform;
                                    //    this.rockT = this.throwRock.transform;
                                    //    this.rockT.localScale = baseT.localScale;
                                    //    this.rockT.position -= (Vector3)((this.rockT.forward * 2.5f) * 3);
                                    ...
                                    //    //}
                                    //    this.rockT.parent = this.upper_arm_R;
                                    //    //if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && basePV.isMine)
                                    //    //{
                                    //    object[] objArray7 = new object[] { basePV.viewID, baseT.localScale, this.rockT.localPosition, 1f };
                                    //    this.throwRock.GetComponent<PhotonView>().RPC("initRPC", PhotonTargets.Others, objArray7);
                                    //    //}
                                    //    Vector3 vector6;
                                    //    //TITAN.RockThrow(out vector6, targetHero.GetComponent<HERO>());
                                    //    float num3 = 1f;
                                    //    float num4 = -20f;
                                    //    if (this.targetHero != null)
                                    //    {
                                    //        vector6 = ((Vector3)((this.targetHero.transform.position - this.rockT.position) / num3)) + this.targetHero.rigidbody.velocity;
                                    //        float num5 = this.targetHero.transform.position.y + (2f * 1f);
                                    ...
                                    //    }
                                    //    //RockThrow(out vector6, targetHero);
                                    //    //BRM.StatsTab.AddLine("load to :" + vector6, StatsTab.DebugType.LOG);
                                    //    this.throwRock.launch(vector6, this.basePV.owner.uiname, this.basePV.viewID);
    ...
    public GameObject getNearestHeroFocusControl()
    {
        GameObject obj2 = null;
        float positiveInfinity = float.PositiveInfinity;
        ...
        foreach (HERO hero in FengGameManagerMKII.heroes)
        {
            ...
            if (num2 < positiveInfinity && num2 > 1)
            {
                obj2 = gameObject;
                positiveInfinity = num2;
                StatsTab.AddLine("Set focus to " + hero.name, StatsTab.DebugType.LOG);
            }
        }
        return obj2;
    }
        ...
        if (this.skillId == "bomb")
        {
            if (FengCustomInputs.Inputs.isInputDown[InputCode.attack1] && (this.skillCDDuration <= 0f))
            {
                if (FengGameManagerMKII.RandomizeBombColor)
                {
                    //System.Random rnd = new System.Random();
                    ExitGames.Client.Photon.Hashtable propertiesToSet = new ExitGames.Client.Photon.Hashtable();
                    propertiesToSet.Add(PhotonPlayerProperty.RCBombR, UnityEngine.Random.Range(0.0f, 1.0f));
                    propertiesToSet.Add(PhotonPlayerProperty.RCBombG, UnityEngine.Random.Range(0.0f, 1.0f));
                    propertiesToSet.Add(PhotonPlayerProperty.RCBombB, UnityEngine.Random.Range(0.0f, 1.0f));
                    propertiesToSet.Add(PhotonPlayerProperty.RCBombA, UnityEngine.Random.Range(0.0f, 1.0f));
                    PhotonNetwork.player.SetCustomProperties(propertiesToSet);
                }
                ...
                /////
                //AIMed 
                if (targetHero == null && FengGameManagerMKII.AIMBotBomb) targetHero = getNearestHeroFocusControl();
                RaycastHit hitInfo = new RaycastHit();
                Ray ray = FengGameManagerMKII.AIMBotBomb ? new Ray(baseT.position, targetHero.transform.position - baseT.position) : Camera.main.ScreenPointToRay(Input.mousePosition);
                LayerMask mask = ((int)1) << LayerMask.NameToLayer("Ground");
                LayerMask mask2 = ((int)1) << LayerMask.NameToLayer("EnemyBox");
                LayerMask mask3 = mask2 | mask;
                ...
                if (FengGameManagerMKII.AIMBotBomb && targetHero != null)
                {
                    vect = Vector3.Normalize(this.targetV - this.currentV);
                    Vector3 pos = targetHero.transform.position + targetHero.rigidbody.velocity * Vector3.Distance(baseT.position, targetHero.transform.position) / bombSpeed;
                    GameObject bemb = PhotonNetwork.Instantiate("RCAsset/BombMain", this.currentV + ((Vector3)(vect * 4f)), new Quaternion(pos.x, pos.y, pos.z, 1f), 0);
                    //vect = ((Vector3)((targetHero.transform.position - bemb.transform.position) / num3)) + targetHero.rigidbody.velocity;
                    ////float num5 = targetHero.transform.position.y + (2f * this.myLevel); 
                    //float num6 = bemb.transform.position.y; 
                    //vect = new Vector3(vect.x, (num6 / num3) - ((0.5f * num4) * num3), vect.z); 
                    bemb.rigidbody.velocity = (Vector3)(vect * this.bombSpeed);
            ...
            else if ((this.myBomb != null) && !this.myBomb.disabled)
            {
                ...
                bool flag2 = false;
                if (FengGameManagerMKII.AutoExplodeBomb)
                {
                    if (((Vector3.Distance(targetHero.transform.position, myBomb.transform.position) < bombRadius)))
                    {
                        this.myBomb.Explode(this.bombRadius);
                    }
                }
