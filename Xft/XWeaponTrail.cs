    ...
    public class XWeaponTrail : MonoBehaviour
    {
        ...
        private void UpdateVertex()
        {
            ...
            for (int i = 0; i < this.Granularity; i++)
            {
                ...
                pool.Colors[index] = Color.Lerp(Color.black, Color.red, 0.25f);
                zero.x = 0f;
                zero.y = num3;
                pool.UVs[index] = zero;
                pool.Vertices[index + 1] = vector2;
                pool.Colors[index + 1] = Color.Lerp(Color.black, Color.blue, 0.25f);
                zero.x = 0.5f;
                zero.y = num3;
                pool.UVs[index + 1] = zero;
                pool.Vertices[index + 2] = vector5;
                pool.Colors[index + 2] = Color.Lerp(Color.black, Color.cyan, 0.25f);
                zero.x = 1f;
                zero.y = num3;
