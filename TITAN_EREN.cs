...
public class TITAN_EREN : MONO
{
    public TITAN_EREN() : base(SPECIES.TitanEren)
    {
    }
private string attackAnimation;
    ...
    private void Awake()
    {
        //this.species = SPECIES.TITAN_EREN;
    ...
    public void lateUpdate()
    {
        if (((!IN_GAME_MAIN_CAMERA.isPausing || (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE)) && !this.rockLift) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine))
        {
            ...
            Quaternion to = Quaternion.Euler(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y, 0f);
            transform.rotation = Quaternion.Lerp(transform.rotation, to, Time.deltaTime * 2f);
        }
    }
    ...
    private void removeMe(PhotonMessageInfo info)
    {
        ...
        {
            FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "removing erenTitan");
            return;
        }
